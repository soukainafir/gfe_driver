...
static
unique_ptr<double[]> do_pagerank(graph_csrpp* G_MM, uint64_t num_nodes, uint64_t num_iterations, double damping_factor, utility::TimeoutService& timer) {
    // init
...
void CSRPP::pagerank(uint64_t num_iterations, double damping_factor, const char* dump2file){
    ...
    unique_ptr<double[]> ptr_rank = do_pagerank(G_MM, num_nodes, num_iterations, damping_factor, tcheck);
     printf("pass 0.0\n");
